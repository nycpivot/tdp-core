...
describe('User Page', () => {
  ...
  it('should render the overview tab', async () => {
    ...
    expect(await screen.findByText(/jdoe/i)).toBeInTheDocument();
    expect(await screen.findByText(/Jenny Doe/i)).toBeInTheDocument();
  });

  it('should render new tabs', async () => {
    const surface = new EntityPageSurface();
    surface.userPage.addTab(
      <EntityLayout.Route path="/new-tab" title="New Tab">
        <Grid container spacing={3}>
          <Grid item xs={12}>
            <div>I am a new user tab</div>
          </Grid>
        </Grid>
      </EntityLayout.Route>,
    );
    ...
    expect(await screen.findByText(/i am a new user tab/i)).toBeInTheDocument();
  });
});
