...
export const GoogleResolverPlugin: BackendPluginInterface = () => surfaces => {
  const GoogleSignInResolver: SignInResolver<OAuthResult> =
  async (info, context): Promise<BackstageSignInResult> => {
    const email = info.profile.email;
    if (!email) {
      throw new Error('Google login failed, user profile does not contain an email');
    }
...
async function signInWithEmail(email: string, context: AuthResolverContext): Promise<BackstageSignInResult> {
  ...
  surfaces.applyTo(SignInResolverSurface, surface => {
    surface.add('google', GoogleSignInResolver);
  });
};
