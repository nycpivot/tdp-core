...
export const BitbucketResolverPlugin: BackendPluginInterface = () => surfaces => {
  const BitbucketSignInResolver: SignInResolver<OAuthResult> =
  async (info, context): Promise<BackstageSignInResult> => {
    const { fullProfile } = info.result;
    const userId = fullProfile.username;
    if (!userId) {
      throw new Error('Bitbucket login failed, user profile does not contain a username');
    }
...
async function signInWithName(name: string, context: AuthResolverContext): Promise<BackstageSignInResult> {
  ...
  surfaces.applyTo(SignInResolverSurface, surface => {
    surface.add('bitbucket', BitbucketSignInResolver);
  });
};
