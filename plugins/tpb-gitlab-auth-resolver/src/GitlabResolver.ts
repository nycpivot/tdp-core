...
export const GitlabResolverPlugin: BackendPluginInterface = () => surfaces => {
  const GitlabSignInResolver: SignInResolver<OAuthResult> =
  async (info, context): Promise<BackstageSignInResult> => {
    const email = info.profile.email;
    if (!email) {
      throw new Error('Gitlab login failed, user profile does not contain an email');
    }
...
async function signInWithEmail(email: string, context: AuthResolverContext): Promise<BackstageSignInResult> {
  ...
  surfaces.applyTo(SignInResolverSurface, surface => {
    surface.add('gitlab', GitlabSignInResolver);
  });
};
