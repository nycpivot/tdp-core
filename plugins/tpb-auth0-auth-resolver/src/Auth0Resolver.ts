...
export const Auth0ResolverPlugin: BackendPluginInterface = () => surfaces => {
  const Auth0SignInResolver: SignInResolver<OAuthResult> =
  async (info, context): Promise<BackstageSignInResult> => {
    const email = info.profile.email;
    if (!email) {
      throw new Error('Auth0 login failed, user profile does not contain an email');
    }
...
async function signInWithEmail(email: string, context: AuthResolverContext): Promise<BackstageSignInResult> {
  ...
  surfaces.applyTo(SignInResolverSurface, surface => {
    surface.add('auth0', Auth0SignInResolver);
  });
};
