...
import { AuthResolverContext, OidcAuthResult, SignInResolver } from '@backstage/plugin-auth-backend';
import { BackstageSignInResult } from '@backstage/plugin-auth-node';

export const OidcResolverPlugin: BackendPluginInterface = () => surfaces => {
  const OidcSignInResolver: SignInResolver<OidcAuthResult> =
  async (info, context): Promise<BackstageSignInResult> => {
    const email = info.profile.email;
    if (!email) {
      throw new Error('OIDC login failed, user profile does not contain an email');
    }
...
async function signInWithEmail(email: string, context: AuthResolverContext): Promise<BackstageSignInResult> {
  ...
  surfaces.applyTo(SignInResolverSurface, surface => {
    surface.add('oidc', OidcSignInResolver);
  });
};
