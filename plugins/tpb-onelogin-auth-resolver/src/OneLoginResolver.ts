...
export const OneLoginResolverPlugin: BackendPluginInterface = () => surfaces => {
  const OneLoginSignInResolver: SignInResolver<OAuthResult> =
async (info, context): Promise<BackstageSignInResult> => {
  const email = info.profile.email;
  if (!email) {
    throw new Error('OneLogin login failed, user profile does not contain an email');
  }
...
async function signInWithEmail(email: string, context: AuthResolverContext): Promise<BackstageSignInResult> {
  ...
  surfaces.applyTo(SignInResolverSurface, surface => {
    surface.add('onelogin', OneLoginSignInResolver);
  });
};
